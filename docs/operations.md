...
| `REQ-GPS-ACTUAL-01` | PX4 GNSS fusion drops after disabling GPS aiding |
| `REQ-VIO-FUSION-01` | External-vision position remains fused during GPS loss |

### GPS-aiding-loss estimator demonstration

The `gps-denied` scenario denies GNSS to the EKF inside the visible prism and
exercises PX4 estimator integration using simulated external odometry under GPS-aiding loss. The odometry is derived from Gazebo vehicle pose with modeled noise/drift; it is not camera localization, visual SLAM, or visual-inertial odometry. Legacy module names and parameter names containing `vio` are retained for compatibility.
...
- CSV/HUD evidence records GPS failure state, external-odometry stream health, and GNSS/EV fusion flags
- V&V independently checks GNSS fusion drop and continuous external-vision fusion

Simulator ground truth is used only as a simulated external-odometry input. Guidance
...
## Hardware integration is unvalidated

These are connection starting points, not a deployment procedure. No hardware or HIL evidence is included. Airframe integration, real sensors and odometry, timing, and flight validation remain separate work.
