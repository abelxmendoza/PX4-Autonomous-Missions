...
This demo is the final scene of a longer engineering story. An earlier version of the cooperative coordinator reported every task complete while the recorded trajectory showed the surviving drone had turned away roughly 0.8–0.9 m from several task points; independent verification rejected that mission, the acceptance logic was fixed, and the verifier was not weakened ([the full case study](case_studies/swarm-endpoint-acceptance.md)). The recording narrated below is the fresh flight after the fix — the demo is not "takeoff, waypoints, landing" but the proof that fault handling, recovery, and independently verified completion now hold together.

The story beats, in playback time: two drones begin the cooperative mission and divide the work (0:00) → one vehicle is commanded to retire (~0:18) → the coordinator confirms continuous landed/disarmed evidence before releasing its work (~0:28–0:29) → the remaining vehicle inherits the unfinished tasks (~0:29) and flies to the reassigned endpoints → four task visits are recorded as observed positions (~0:41–0:54) → the surviving vehicle lands and the mission reports completion (~1:04) → the verifier analyzes the recorded trajectory and produces the PASS/FAIL evidence, and the replay highlights task visits and minimum separation. Total length: about 64 seconds (a 32-second overview at 2×).
